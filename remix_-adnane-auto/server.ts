...
async function startServer() {
  ...
  app.use((req, res, next) => {
    ...
    // Bypass payload security checks entirely for authenticated admin endpoints or asset requests
    // Admin reads logs or updates car info, we must NEVER block them!
    const token = req.query.token || req.body?.token;
    const isAdminRequest = (typeof token === 'string' && activeAdminSessions.has(token)) || req.path.startsWith('/api/admin/security');
